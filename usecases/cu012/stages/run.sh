...
eventlog=$VARDIR/lib/luids/event/security-events.json
[ -f $eventlog ] || die "$eventlog doesn't exists!"
...
test02() {
    ...
    cat $eventlog | grep "www.facebook.com" | grep 'level":"info"'  &>>$testlog
    [ $? -ne 0 ] && step_err && return 1
...
test03() {
    ...
    cat $eventlog | grep "www.xvideos.com" | grep 'level":"low"'  &>>$testlog
    [ $? -ne 0 ] && step_err && return 1
...
test04() {
    ...
    cat $eventlog | grep "www.badsite.com" | grep 'level":"medium"'  &>>$testlog
    [ $? -ne 0 ] && step_err && return 1
    ...
    cat $eventlog | grep "54.37.157.73" | grep 'level":"medium"'  &>>$testlog
    [ $? -ne 0 ] && step_err && return 1
